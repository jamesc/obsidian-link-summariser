...
from summarize_links.extract.fetching import _is_javascript_required
...
class TestIsJavascriptRequired:
    """Tests for JavaScript-required page detection."""

    def test_returns_false_for_normal_content(self) -> None:
        """Should return False for pages with real content."""
        html = """
        <html>
        <head><title>Article</title></head>
        <body>
            <h1>Real Content</h1>
            <p>This is a paragraph with meaningful text content that a reader would see.</p>
            <p>Another paragraph with more information about the topic.</p>
        </body>
        </html>
        """
        assert _is_javascript_required(html) is False

    def test_returns_true_for_js_required_message_small_page(self) -> None:
        """Should return True for small pages with JS-required message."""
        html = """
        <html>
        ...
        """
        assert _is_javascript_required(html) is True

    def test_returns_true_for_please_enable_javascript(self) -> None:
        """Should detect 'please enable javascript' message."""
        html = """
        <html><body>
    ...
    def test_returns_true_for_javascript_is_required(self) -> None:
        """Should detect 'javascript is required' message."""
        html = """
        <html><body>
    ...
    def test_returns_false_without_js_message(self) -> None:
        """Should return False if no JS-required message present."""
        html = """
        <html><body>
            <div id="root"></div>
    ...
    def test_returns_true_for_spa_shell_with_scripts(self) -> None:
        """Should return True for SPA shell with JS message and script tags."""
        html = """
        <html>
    ...
    def test_returns_true_for_spa_shell_with_styles(self) -> None:
        """Should return True for SPA shell with JS message and style tags."""
        html = """
        <html>
        <head><title>SPA</title></head>
        <body>
    ...
    def test_returns_false_for_large_page_with_js_message_but_content(self) -> None:
        """Should return False if page has JS message but also substantial real content."""
        # Build a page with JS message but substantial body text (>10KB total, >200 chars text)
        # Need enough content to exceed the 10KB threshold and 200 char text threshold
        long_paragraph = "This is a real paragraph with meaningful content. " * 20
        content_paragraphs = f"<p>{long_paragraph}</p>" * 10
        html = f"""
        <html>
        <head><title>Article</title></head>
        <body>
        ...
        """
        # Verify the page is large enough (>10KB)
        assert len(html) > 10000, f"Test page too small: {len(html)} bytes"
        assert _is_javascript_required(html) is False

    def test_handles_missing_body_tag(self) -> None:
        """Should handle HTML without body tag gracefully."""
        html = "<html><head><title>No Body</title></head></html>"
        assert _is_javascript_required(html) is False

    def test_case_insensitive_detection(self) -> None:
        """Should detect JS messages case-insensitively."""
        html = """
        <html><body>
